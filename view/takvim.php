...
class Calendar{
    ...
    **/
    public function __construct() {
		if(empty($_GET["id"]))
		{
			$this->naviHref = htmlentities($_SERVER['PHP_SELF'].'?sayfa='.$_GET["sayfa"].'');
		}
		else
		{
			$this->naviHref = htmlentities($_SERVER['PHP_SELF'].'?sayfa='.$_GET["sayfa"].'&id='.$_GET["id"].'');
		}
    }
    /********************* PROPERTY ********************/
    private $dayLabels = array("Pzt", "Sal", "Çrş", "Prş", "Cum", "Cmt", "Paz");
    ...
    **/
    private function _createNavi() {
      ...
      return '<div class="calendar_header">' . "\r\n" . '<a class="calendar_prev" href="' . $this->naviHref . '&amp;month=' . sprintf('%02d', $preMonth) . '&amp;year=' . $preYear.'">Önceki</a>' . "\r\n" . '<span class="calendar_title">' . date('M Y', strtotime($this->currentYear . '-' . $this->currentMonth . '-1')) . '</span>' . "\r\n" . '<a class="calendar_next" href="' . $this->naviHref . '&amp;month=' . sprintf("%02d", $nextMonth) . '&amp;year=' . $nextYear . '">Sonraki</a>' . "\r\n"  . '</div>';
    }
    /**
